...
def preprocess_vistral(
    ...
    assert conv.sep_style == conversation_lib.SeparatorStyle.VISTRAL
...
def preprocess(
    ...
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.VISTRAL: 
        return preprocess_vistral(sources, tokenizer, has_image=has_image)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
